...
resource "oci_core_virtual_network" "quobyte" {
  count          = var.use_existing_vcn ? 0 : 1
  cidr_block     = var.vpc_cidr
  compartment_id = var.compartment_ocid
  display_name   = "quobyte"
  dns_label      = "quobyte"
}
...
resource "oci_core_internet_gateway" "internet_gateway" {
  ...
  vcn_id         = oci_core_virtual_network.quobyte[0].id
}
...
resource "oci_core_subnet" "public" {
  ...
  security_list_ids = [oci_core_virtual_network.quobyte[0].default_security_list_id, oci_core_security_list.public_security_list[0].id]
  dhcp_options_id   = oci_core_virtual_network.quobyte[0].default_dhcp_options_id
  dns_label         = "public"
}
...
resource "oci_core_subnet" "storage" {
  ...
  security_list_ids          = [oci_core_virtual_network.quobyte[0].default_security_list_id, oci_core_security_list.private_security_list[0].id]
  dhcp_options_id            = oci_core_virtual_network.quobyte[0].default_dhcp_options_id
